...
```{r}
readCSVFile <- function(file_Path){
  df <- read.csv(
    file_Path,
    stringsAsFactors = F)
  #View(df)
  df$created <-  strptime(df$created,format="%m/%d/%Y %H:%M", tz="GMT")
...
# df.data.science.tweet <- readCSVFile("/Users/ritesh/pad-datascience/R/unstructureData/data/combinedDSNew.csv")
...
df.hashtag_words <- freq_words[grepl('^#', as.character(freq_words$words)), ]
...
quarterly.GrpWord.List$words <- 
  lapply(lapply(quarterly.GrpWord.List$words, unlist), FUN = function(x2) x2[grepl('^#', as.character(x2))])
